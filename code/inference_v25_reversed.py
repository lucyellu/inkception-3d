...
def find_image_file(dataset_path, fragment_id, index):
    # Search for files with varying lengths of digits in the filename
    search_patterns = [
        f"{dataset_path}/{fragment_id}/layers/{index:02d}.tif",
        f"{dataset_path}/{fragment_id}/layers/{index:03d}.tif",
        f"{dataset_path}/{fragment_id}/layers/{index}.tif"
    ]
...
#16 layers 16 to 32
start_idx: int = 16  # Default value
end_idx: int = 32    # Default value
in_chans: int = 16   # Default value



def get_next_filename(directory, base_filename, size, batch_size, extension):
    iteration = 1
    while True:
        new_filename = f"{base_filename}_size{size}_batchsize{batch_size}_{iteration:02d}.{extension}"
...
def read_image_mask(fragment_id, start_idx, end_idx, rotation=0):
    ...
    for i in idxs:
        file_path = find_image_file(dataset_path, fragment_id, i)
        if file_path:
            ...
            print(f"Image file not found for index {i}")
            return None, None
    ...
    fragment_mask = None
    mask_path = f'{dataset_path}/{fragment_id}/{fragment_id}_mask.png'
    if os.path.exists(mask_path):
        fragment_mask = cv2.imread(mask_path, 0)
        pad0 = (CFG.tile_size - fragment_mask.shape[0] % CFG.tile_size)
        pad1 = (CFG.tile_size - fragment_mask.shape[1] % CFG.tile_size)
...
# Define the fragment_id and get start_idx, end_idx from args
fragments = os.listdir('./eval_segments_shared')
...
# Define the base directory including fragment_id, start_idx, end_idx, and timestamp
base_dir = f'./outputs/{fragment_id}_{start_idx}to{end_idx}_{timestamp}'
...
def predict_fn(test_loader, model, device, test_xyxys, pred_shape):
    ...
    checkpoints = [total_steps // 100]  # For 1%
    #checkpoints.extend([total_steps * p // 100 for p in range(10, 101, 10)])  # For every 10% after 1%
    checkpoints.extend([total_steps * p // 100 for p in range(25, 101, 25)])  # For every 10% after 1%
    # total_steps = len(test_loader)
    # checkpoints = [total_steps * p // 100 for p in range(2, 101, 2)]




    for step, (images, xys) in tqdm(enumerate(test_loader), total=total_steps):
        ...
        for i, (x1, y1, x2, y2) in enumerate(xys):
            interpolated_pred = np.multiply(F.interpolate(y_preds[i].unsqueeze(0).float(), scale_factor=4, mode='bilinear').squeeze(0).squeeze(0).numpy(), kernel)
            mask_pred[y1:y2, x1:x2] += interpolated_pred
            mask_count[y1:y2, x1:x2] += np.ones((CFG.size, CFG.size))


        if step in checkpoints:
            progress_percentage = (step * 100) // total_steps
            temp_mask = np.divide(mask_pred, mask_count, out=np.zeros_like(mask_pred), where=mask_count != 0)
            temp_mask = np.clip(np.nan_to_num(temp_mask), a_min=0, a_max=1)
            ...
            progress_filename = f'{full_dir}/{fragment_id}_{args.start_idx}to{args.end_idx}_reversed_stride{args.stride}_size{args.size}_batchsize{args.batch_size}_progress_{progress_percentage}percent.png'
        ...
        #     # Update progress_filename format
        #     progress_filename = f'{full_dir}/{fragment_id}_{start_idx}to{end_idx}_progress_{progress_percentage}percent.png'
        #     temp_mask_image.save(progress_filename)
        #     print(f"Progress image saved at step {step} ({progress_percentage}%): {progress_filename}")
...
#test_loader, test_xyxz, test_shape, fragment_mask = get_img_splits(fragment_id, 48, 64, 0)
#test_loader, test_xyxz, test_shape, fragment_mask = get_img_splits(fragment_id, args.start_idx, args.start_idx + 16, 0)
...
mask_pred_image = Image.fromarray(mask_pred)



# Get the next available filename including size and batch size
base_filename = f"{fragment_id}_{args.start_idx}to{args.end_idx}_stride{args.stride}_reversed"
extension = 'png'
