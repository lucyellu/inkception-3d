...
class InferenceArgumentParser(Tap):
    segment_id: list[str] = ['20231210132040']
    #segment_path: str = './scrolls'
    segment_path: str = './eval_segments_shared'
    model_path: str = './models/timesformer_wild15_20230702185753_0_fr_i3depoch=12.ckpt'

    out_path: str = "./predictions"
    #out_path: str = '/content/drive/MyDrive/A_Scroll/inkception-3d/predictions'
    stride: int = 32
    start: int = 32
    num_layers: int = 1
    workers: int = 8
...
def read_image_mask(fragment_id, start_idx, end_idx, rotation=0):
    ...
    for i in idxs:
        ...
        # Check for both 2 and 3 digit files
        if os.path.exists(f"{args.segment_path}/{fragment_id}/layers/{i:02}.{args.format}"):
            file_path = f"{args.segment_path}/{fragment_id}/layers/{i:02}.{args.format}"
        elif os.path.exists(f"{args.segment_path}/{fragment_id}/layers/{i:03}.{args.format}"):
            file_path = f"{args.segment_path}/{fragment_id}/layers/{i:03}.{args.format}"
        else:
            print(f"ERROR: File for index {i} not found in both 2 and 3 digit formats.")
            continue
    ...
    if args.reverse != 0 or fragment_id in [
        '20230701020044', 'verso', '20230901184804', '20230901234823', '20230531193658', '20231007101615', '20231005123333', '20231011144857',
        '20230522215721', '20230919113918', '20230625171244', '20231022170900', '20231012173610', '20231016151000'
    ]:
    ...
    for fragment_id in args.segment_id:
        if glob.glob(f"{args.segment_path}/{fragment_id}/layers/*.tif"):
            ...
            if len(args.out_path) > 0:
                ...
                #replaced timestamp with stride and size
                cv2.imwrite(os.path.join(
                    f"{args.out_path}/{fragment_id}",
                    f"{fragment_id}_{start_f}to{end_f}_n{args.num_layers}layers_stride{CFG.stride}_size{CFG.size}_normal_timesformer753.png"),
                    image_cv
                )
