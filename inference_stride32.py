...
class InferenceArgumentParser(Tap):
    ...
    model_path: str = '/content/drive/MyDrive/A_Scroll/inception-3d/models/valid_20230827161847_0_fr_i3depoch=7.ckpt'
...
def predict_fn(test_loader, model, device, test_xyxys, pred_shape):
    ...
    checkpoints.extend([total_steps * p // 100 for p in range(5, 101, 5)])  # For every 10% after 1%
    # total_steps = len(test_loader)
    ...
    for step, (images, xys) in tqdm(enumerate(test_loader), total=total_steps):
        ...
        if step in checkpoints:
            ...
            progress_filename = f'{full_dir}/{fragment_id}_{args.start_idx}to{args.end_idx}_stride{args.stride}_size{args.size}_batchsize{args.batch_size}_progress_{progress_percentage}percent.png'
